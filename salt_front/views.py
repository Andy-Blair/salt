    ...
    if request.is_websocket():
        for m in request.websocket:
            ...
            for i in web_server_ip:
                ...
                sync_re = cli.cmd(tgt=i.strip(), fun='state.sls', arg=['pkg.scripts.web_git.%s.%s_git' % (sls_name, sls_name)] )
                print "update:",sync_re
    ...
    if request.method == "GET":
        ...
        for num in range(sls_git_flen):
            if "- name:" in sls_git_lines[num]:
                if rec_data['apptype'] == "IIS":
                    sls_git_lines[num] = "    " + sls_git_lines[num].strip() + " d:/product/web_git/%s.py\n" % pyscript_name
                else:
                    sls_git_lines[num] = "    " + sls_git_lines[num].strip() + " /apps/product/web_git/%s.py\n" % pyscript_name
                    sls_git_lines.append("    - user: app")
        new_git_sls = open("/srv/salt/pkg/scripts/web_git/%s/%s_git.sls" % (pyscript_name,pyscript_name), "w")
        new_git_sls.writelines(sls_git_lines)
        new_git_sls.close()
        top_sls = open("/srv/salt/top.sls", "a+")
        top_data = ["    - pkg.scripts.web_git.%s.%s\n" % (pyscript_name,pyscript_name),"    - pkg.scripts.web_git.%s.%s_git\n" % (pyscript_name,pyscript_name)]
        ...
        for ip in rec_data['serverip'].split(','):
            ...
            print cli.cmd(tgt=ip, fun="state.sls", arg=["pkg.scripts.web_git.%s.%s" % (pyscript_name,pyscript_name)])
